...
from booking_fetch import prepare_manual_capture_request
...
def _fetch_window(
    ...
    for i, lst in enumerate(listings, 1):
        ...
        print(f"    [{i:>2}/{len(listings)}] {'OK' if ok else 'WARN'}", end="\r")
        if i < len(listings):
            time.sleep(2.0)
    print()
    return listings
...
def collect_data(
    ...
    try:
        wd_raw = _fetch_window(query, wd_in, wd_out, wd_label, geo, session, max_results, max_pages)
        we_raw = _fetch_window(query, we_in, we_out, we_label, geo, session, max_results, max_pages)
        ws_raw = _fetch_window(query, ws_in, ws_out, ws_label, geo, session, max_results, max_pages)
    except Exception as exc:
        msg = str(exc)
        manual_needed = (
            "manual capture request" in msg
            or "blocked automated requests" in msg
            or "Save the real search-results page as" in msg
            or "verification page" in msg
        )
        if not manual_needed:
            raise
        lines = [
            "Booking.com manual capture required for market mode.",
            "Save these 3 search-result pages, then run the same M mode analysis again.",
            "",
        ]
        for ci, co, label in windows:
            info = prepare_manual_capture_request(
                query,
                ci,
                co,
                page=1,
                reason=f"market mode {label} capture requested",
            )
            lines.extend([
                f"[{label}] {ci} ~ {co}",
                f"Open URL shortcut: {info['shortcut']}",
                f"Save the real search-results page as: {info['html']}",
                "",
            ])
        raise RuntimeError("\n".join(lines)) from exc
...
def compute_effective_adr(listings: list[dict]) -> dict:
    """총가(= 1박가 + 청소비 + Booking.com 서비스료) 기준 체감 ADR."""
...
def _round_1k(value: float) -> int:
    return round(value / 1000) * 1000


def build_price_recommendations(
    stats: dict,
    premium: float,
    demand: dict | None = None,
    cleaning_fee: int = 80_000,
    avg_stay_nights: float = 2.0,
) -> dict:
    """추천가를 산출한다.

    쉬운 설명:
    - 먼저 시장 가격을 낮음/중간/높음 3단계로 나눈다.
    - 수요가 좋으면 조금 올리고, 경쟁이 빡세면 조금 내린다.
    - 청소비가 높으면 손님이 보는 총액이 비싸지므로 1박가를 조금 낮춘다.
    - 표본이 적으면 욕심내지 않도록 보수적으로 잡는다.
    """
    demand = demand or {}
    count = int(stats.get("clean_count") or stats.get("count") or 0)
    avg_rating = float(stats.get("avg_rating") or 0)
    avg_reviews = float(demand.get("avg_review_count") or 0)
    base_adr = float(stats.get("mean_clean") or stats.get("mean") or 0)

    if avg_reviews >= 35:
        demand_adj = 0.04
        demand_text = "리뷰가 많은 편이라 수요가 비교적 확인됩니다."
    elif avg_reviews >= 15:
        demand_adj = 0.02
        demand_text = "리뷰가 어느 정도 있어 기본 수요는 있어 보입니다."
    elif avg_reviews >= 5:
        demand_adj = 0.00
        demand_text = "리뷰 수가 보통 수준이라 시장 중간값을 기준으로 봅니다."
    else:
        demand_adj = -0.04
        demand_text = "리뷰가 적어 수요 확인이 약하므로 처음에는 낮게 잡습니다."

    prem_pct = premium * 100
    if prem_pct >= 25:
        weekend_adj = 0.03
        weekend_text = "주말 가격이 강해 주말 수요가 좋은 편입니다."
    elif prem_pct >= 10:
        weekend_adj = 0.01
        weekend_text = "주말 가격이 약간 높아 주말 수요가 있습니다."
    elif prem_pct <= -5:
        weekend_adj = -0.03
        weekend_text = "주말 가격이 약해 무리한 인상은 피합니다."
    else:
        weekend_adj = 0.00
        weekend_text = "주말과 평일 차이가 크지 않습니다."

    if avg_rating >= 4.85:
        quality_adj = -0.04
        quality_text = "경쟁 숙소 평점이 높아 새 숙소는 초반 할인이 필요합니다."
    elif avg_rating >= 4.70:
        quality_adj = -0.02
        quality_text = "경쟁 숙소 품질이 괜찮아 가격을 너무 높게 잡지 않습니다."
    elif avg_rating > 0:
        quality_adj = 0.02
        quality_text = "경쟁 숙소 평점이 아주 높지는 않아 차별화 여지가 있습니다."
    else:
        quality_adj = 0.00
        quality_text = "평점 정보가 부족해 품질 조정은 하지 않습니다."

    if count < 10:
        sample_adj = -0.06
        confidence = "낮음"
        sample_text = "비교 숙소가 적어 가격을 보수적으로 잡았습니다."
    elif count < 25:
        sample_adj = -0.03
        confidence = "보통"
        sample_text = "비교 숙소가 많지는 않아 약간 보수적으로 잡았습니다."
    else:
        sample_adj = 0.00
        confidence = "높음"
        sample_text = "비교 숙소 수가 충분해 시장 가격을 비교적 믿을 수 있습니다."

    cleaning_per_night = cleaning_fee / max(avg_stay_nights, 1.0)
    cleaning_ratio = cleaning_per_night / base_adr if base_adr else 0
    if cleaning_ratio >= 0.25:
        cleaning_adj = -0.06
        cleaning_text = "청소비 부담이 커서 손님이 보는 총액을 낮추도록 1박가를 내렸습니다."
    elif cleaning_ratio >= 0.15:
        cleaning_adj = -0.03
        cleaning_text = "청소비가 약간 높아 1박가를 조금 낮췄습니다."
    else:
        cleaning_adj = 0.00
        cleaning_text = "청소비 부담이 크지 않아 가격 조정을 하지 않았습니다."

    total_adj = _clamp(
        demand_adj + weekend_adj + quality_adj + sample_adj + cleaning_adj,
        -0.14,
        0.10,
    )
    weekend_factor = _clamp(premium + max(weekend_adj, 0), 0.0, 0.55)

    tier_defs = [
        ("starter", "스타터", "리뷰 확보", stats.get("p40", 0), total_adj - 0.02),
        ("standard", "표준", "가장 먼저 써볼 기준가", stats.get("p55", 0), total_adj),
        ("premium", "프리미엄", "후기와 사진이 충분할 때", stats.get("p70", 0), total_adj + 0.02),
    ]
    tiers = []
    for key, name, purpose, anchor, tier_adj in tier_defs:
        weekday = _round_1k(max(0, float(anchor or 0) * (1 + _clamp(tier_adj, -0.18, 0.12))))
        weekend = _round_1k(weekday * (1 + weekend_factor))
        tiers.append({
            "key": key,
            "name": name,
            "purpose": purpose,
            "weekday": weekday,
            "weekend": weekend,
            "base": _round_1k(float(anchor or 0)),
        })

    return {
        "tiers": tiers,
        "standard_weekday": next(t["weekday"] for t in tiers if t["key"] == "standard"),
        "standard_weekend": next(t["weekend"] for t in tiers if t["key"] == "standard"),
        "total_adjustment_pct": round(total_adj * 100, 1),
        "weekend_factor_pct": round(weekend_factor * 100, 1),
...
def _build_recommendation_sheet(wb, f, stats, premium, price_recs: dict | None = None):
    ...
    if price_recs is None:
        price_recs = build_price_recommendations(stats, premium)
    tiers = price_recs["tiers"]

    # 평일 추천가 헤더
    ws.merge_range(row, 1, row, 4, "평일 추천가", f["section"])
    ws.set_row(row, 22); row += 1

    for col_start, tier in enumerate(tiers):
        c = 1 + col_start
        ws.write(row, c, tier["name"], f["rec_tier"])
    ws.set_row(row, 28); row += 1

    for col_start, _tier in enumerate(tiers):
        c = 1 + col_start
    ...
    for col_start, tier in enumerate(tiers):
        c = 1 + col_start
        ws.write(row, c, tier["weekday"], f["rec_price"])
    ws.set_row(row, 55); row += 1

    for col_start, tier in enumerate(tiers):
        c = 1 + col_start
        ws.write(row, c, tier["purpose"], f["rec_desc"])
    ws.set_row(row, 40); row += 2

    # 주말 추천가
    ws.merge_range(row, 1, row, 4, f"주말 추천가 (평일 대비 +{price_recs['weekend_factor_pct']:.1f}%)", f["section"])
    ws.set_row(row, 22); row += 1
    ...
    for col_start, tier in enumerate(tiers):
        c = 1 + col_start
        ws.write(row, c, tier["weekend"], f["rec_price"])
    ws.set_row(row, 55); row += 1
    ...
    # 주의사항
    ws.merge_range(row, 1, row, 8, "가격 조정 이유", f["section"])
    ws.set_row(row, 22); row += 1
    for reason in price_recs["explain"]:
        ws.merge_range(row, 1, row, 8, "- " + reason,
                       wb.add_format({"font_size": 9, "font_color": C_DARK_TEXT, "text_wrap": True}))
        ws.set_row(row, 18); row += 1
    row += 1

    notes = [
        f"※ 추천 신뢰도: {price_recs['confidence']} / 시장 가격에 전체 조정 {price_recs['total_adjustment_pct']:+.1f}% 적용.",
        "※ 본 추천가는 수집 시점 기준이며 계절·이벤트·Booking.com 정책 변동에 따라 달라질 수 있습니다.",
...
def _build_revenue_sheet(wb, f, scenarios, adr_clean, premium, market_score,
    ...
    r += 1
    note = ("주의: 이 수치는 업계 평균 예약률 기반 가정치입니다. 실제 매출과 다를 수 있으며, "
    ...
    if market_score.get("warning"):
        ...
        ws.merge_range(r, 0, r, 3,
            "주의: 샘플 수가 적어(< 15개) 시장 매력도 점수의 신뢰도가 낮습니다.",
            warn_fmt)
...
def _build_demand_sheet(wb, f, demand, listings):
    ...
    r += 1
    warn_txt = ("주의: 이 섹션의 예약률 수치는 저신뢰도 추정치입니다.\n"
...
def _build_guide_sheet(wb, f, is_internal: bool = False) -> None:
    ...
    r = _section(r, "가격 용어")
    r = _item(r, "1박가 (ADR)",
              "Booking.com 검색 결과에 표시되는 1박 기준 숙박료. 청소비·서비스료 미포함.")
    r = _item(r, "총가 (Total Price)",
              "1박가 × 숙박 박수. Booking.com 청소비·서비스료는 별도입니다.")
    r = _item(r, "블렌디드 ADR",
    ...
    r = _item(r, "스타터",
              "처음 리뷰를 모으기 위한 낮은 가격입니다. 경쟁이 강하거나 비교 숙소가 적으면 더 보수적으로 잡습니다.")
    r = _item(r, "표준",
              "가장 먼저 써볼 기준 가격입니다. 수익 시뮬레이터도 이 가격을 기준으로 계산합니다.")
    r = _item(r, "프리미엄",
              "사진, 후기, 운영 품질이 충분할 때 목표할 수 있는 높은 가격입니다.")
    ...
    r = _item(r, "호스트 수수료 3%",
              "Booking.com 기본 호스트 수수료. 총 매출의 3%를 공제합니다.")
    r = _item(r, "청소비 계산",
...
def build_client_report(
    ...
    _build_recommendation_sheet(wb, f, stats, premium, price_recs)
    if scenarios and market_score:
        adr_base = (
            price_recs.get("standard_weekday")
            if price_recs else
            (stats.get("mean_clean") or stats.get("mean") or 0)
        )
        _build_revenue_sheet(wb, f, scenarios, adr_base, premium, market_score,
                             cleaning_fee, avg_stay, monthly_ops)
    _build_guide_sheet(wb, f, is_internal=False)

    wb.close()
    _fix_colors(out_path)
    print(f"[OK] 손님용 저장: {out_path.name}")
...
def _build_meta_sheet(wb, f, query, beds, baths, geo, collected):
    ...
    rows = [
        ...
        ("추천가 기준",     "시장 가격 3단계 + 수요/경쟁/청소비/표본 수 조정",        False),
        ("데이터 소스",     "Booking.com (비공식 크롤링, 개인 분석 목적)",     False),
    ]
...
def build_internal_report(
    ...
    wb.close()
    _fix_colors(out_path)
    print(f"[OK] 내부용 저장: {out_path.name}")
...
def _build_score_html(ms: dict) -> str:
    ...
    warn = ('<p style="font-size:11px;color:#FF3B3B;margin-top:12px">'
            '주의: 샘플 수 부족(15개 미만)으로 신뢰도가 낮습니다.</p>'
...
def _build_simulator_html(scenarios: list[dict], eff_adr: dict | None) -> str:
    ...
    if eff_adr and eff_adr.get("mean"):
        eff_str = (
            ...
            f'<br><span style="font-size:11px;opacity:0.7">총가 = 1박가 + 청소비 + Booking.com 서비스료 (게스트 실부담)</span></div>'
        )
    ...
    return (
        ...
        '주의: 업계 평균 기반 가정치입니다. 임대료·대출 상환은 포함되지 않습니다.</div>\n'
        + eff_str
...
def build_html_report(
    ...
    p70 = stats.get("p70", 0)
    if price_recs is None:
        price_recs = build_price_recommendations(stats, premium, demand=demand)
    ...
    tier_by_key = {t["key"]: t for t in price_recs["tiers"]}
    p40_wd = tier_by_key["starter"]["weekday"];   p40_we = tier_by_key["starter"]["weekend"]
    p55_wd = tier_by_key["standard"]["weekday"];  p55_we = tier_by_key["standard"]["weekend"]
    p70_wd = tier_by_key["premium"]["weekday"];   p70_we = tier_by_key["premium"]["weekend"]
    reason_html = "".join(f"<li>{reason}</li>" for reason in price_recs["explain"])
    ...
    html = (
        ...
        '<div class="tier-desc">리뷰 확보 초기 전략<br>처음 써보기 좋은 낮은 가격</div>\n'
        '</div>\n'
        ...
        '<div class="tier-desc">가장 먼저 써볼 기준가<br>수익 계산 기준 가격</div>\n'
        '</div>\n'
        ...
        '<div class="tier-desc">후기와 사진이 충분할 때<br>높게 받을 수 있는 가격</div>\n'
        '</div>\n'

        '</div>\n'  # tier-grid
        f'<p style="font-size:11px;color:var(--text-3);margin-top:18px;line-height:1.9">'
        f'※ 이상치 제거 후 {stats.get("clean_count", total_cnt)}개 숙소 기준 · 추천 신뢰도 {price_recs["confidence"]}<br>'
        f'※ 시장 가격에 전체 조정 {price_recs["total_adjustment_pct"]:+.1f}% 적용, 주말 가산 {price_recs["weekend_factor_pct"]:.1f}% 적용'
        f'</p>\n'
        '<div class="card" style="margin-top:16px"><h3 style="font-size:14px;margin-bottom:10px">가격 조정 이유</h3>'
        f'<ul style="font-size:13px;line-height:1.9;color:var(--text-2);padding-left:18px">{reason_html}</ul></div>\n'
        '</div>\n'  # section
        ...
        # Footer
        + '<footer>\n<div class="container">\n'
        f'<strong>데이터 출처:</strong> Booking.com (비공식 크롤링, 개인 분석 목적) &nbsp;·&nbsp; '
...
<body>
  <nav class="topbar">
    <div class="topbar-inner">
      <div class="brand">Booking.com Market Report</div>
  ...
  <footer class="footer">
    <div class="shell">
      <strong>데이터 출처:</strong> Booking.com 비공식 수집, 개인 분석 목적<br>
...
def run(
    ...
    if checkin:
        try:
            ...
            print(f"[WARN] --checkin 날짜 형식 오류 ({checkin}), 자동 날짜 사용")

    print(f"\n{'='*60}")
    print(f" Booking.com 시장 분석 리포트 v3 - {query}")
    print(f"{'='*60}")
    ...
    print(f" 청소비: KRW {cleaning_fee:,} / 평균 체류: {avg_nights}박")

    print(f"\n[1/5] 지오코딩: {query}")
    geo = geocode_region(query)
    if geo:
        print(f"      좌표 확인: lat={geo.get('lat')}, lon={geo.get('lon')}")
    else:
        print("      좌표 확인 실패: 지역명 검색만으로 진행합니다.")

    print("\n[2/5] 데이터 수집 (3개 날짜창 x B 모드: 평일/금-토/토-일)")
    ...
    if not listings:
        print("[ERROR] 수집된 숙소가 없습니다. 지역명을 더 넓게 입력해보세요.")
        return None
    ...
    print(f"      평균가: KRW {stats.get('mean', 0):,.0f}")
    print(f"      중앙값: KRW {stats.get('median', 0):,.0f}")
    ...
    center_lat   = float((geo or {}).get("lat") or 0)
    center_lon   = float((geo or {}).get("lon") or 0)
    ...
    revenue_adr  = price_recs["standard_weekday"] or adr_clean
    scenarios    = build_revenue_scenarios(
        adr_clean             = revenue_adr,
        premium               = premium,
        occ_low               = occ_low,
        occ_base              = occ_base,
        occ_high              = occ_high,
    ...
    print(f"      체감 ADR (총가): KRW {eff_adr.get('mean', 0):,.0f}")
    print(f"      추천 표준 평일가: KRW {price_recs['standard_weekday']:,}")
    ...
    print(f"      기준 시나리오 월 영업이익: KRW {scenarios[1]['op_profit']:,}")
    ...
    if output_mode in ("both", "client"):
        out_client = base / f"{query}_시장분석_손님용_{ftag}.xlsx"
        build_client_report(
    ...
    if output_mode in ("both", "internal"):
        out_internal = base / f"{query}_시장분석_내부용_{ftag}.xlsx"
    ...
    if output_mode in ("both", "client"):
        print(f"  XLSX 손님용: {query}_시장분석_손님용_{ftag}.xlsx")
    if output_mode in ("both", "internal"):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Booking.com 시장 분석 리포트 생성기 v3")
