"""
Booking.com 시장 분석 - 콘솔 인터페이스
사용: python cli_app.py  (또는 booking_CLI.exe 더블클릭)
"""
...
def _ask_date(prompt: str, default_offset: int) -> str:
    ...
    while True:
        ...
        try:
            ...
            print("    [WARN] YYYY-MM-DD 형식으로 입력해주세요.")
...
def main() -> None:
    print()
    _hr()
    print("  Booking.com 시장 분석 도구")
    _hr()
    print("  A  기본 수집  - 가격/위치/평점 Excel (~1분)")
    print("  B  상세 수집  - 소개글/편의시설 포함 (~5분)")
    print("  M  시장 분석  - 평일/주말 비교 리포트 (~20~30분)")
    _hr()
