"""
Booking.com 시세 상세 분석 → Excel (B 모드)
...
from booking_fetch import fetch_detail as _booking_fetch_detail
...
def fetch_detail(url: str, session: cf_requests.Session) -> dict:
    return _booking_fetch_detail(url, session)
...
def run(
    ...
    print(f"[2/4] Booking.com 크롤링: {query} ({checkin} ~ {checkout})")
    ...
    print(f"      수집 결과: {len(listings)}개")

    if not listings:
        print("[ERROR] 수집된 숙소가 없습니다.")
        return None
    ...
    for i, lst in enumerate(listings, 1):
        ...
        print("[OK]" if ok else f"[WARN] {detail.get('description', '')[:30]}")
