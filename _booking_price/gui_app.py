...
class App(ttk.Window):
    ...
    def __init__(self) -> None:
        ...
        self.title("Booking.com Market Studio")
    ...
    def _run(self) -> None:
        ...
        self.lbl_step.configure(text="Booking.com 데이터를 요청할 준비를 하고 있습니다.")
        self._clear_log()
    ...
    def _worker(self, inp: dict) -> None:
        ...
        try:
            ...
            self._log(f"Booking.com Market Studio")
            ...
            msg = str(exc)
            if (
                "manual capture required" in msg
                or "blocked automated requests" in msg
                or "Save the real search-results page as" in msg
                or "verification page" in msg
                or "Saved manual page is still" in msg
            ):
                self._log("\nBooking.com manual capture required", "hdr")
                for line in msg.splitlines():
                    self._log(line, "err" if line.startswith("Booking.com blocked") else None)
                self._log("\nAfter saving the HTML file, run the same analysis again.", "hdr")
                self.after(0, lambda: self.lbl_status.configure(text="HTML save required"))
                self.after(0, lambda: self.lbl_sidebar_status.configure(text="HTML save required"))
                return
