...
def api_subway():
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "AirbnbPriceScanner/1.0"})
...
def api_export():
    ...
    query    = (body.get("query") or "airbnb").strip()
    checkin  = (body.get("checkin") or "").strip()
    checkout = (body.get("checkout") or "").strip()
    ...
    filename = f"airbnb_{safe_q}_{checkin}_{checkout}.xlsx"
